...
void SceneParser::parsePerspectiveCamera() {
    ...
    float angle_radians, focalLength = 1, aperture = 0;
    int width, height;
    // read in the camera parameters
    getToken(token);
    assert (!strcmp(token, "{"));
    
    // 由于部分场景不存在 focalLength 和 aperture 参数，因此改为条件判断更鲁棒
    while (true) {
        getToken(token);
        if (!strcmp(token, "center")) {
            ...
            height = readInt();
        } else if (!strcmp(token, "focalLength")) {
    ...
    camera = new PerspectiveCamera(center, direction, up, width, height, angle_radians, focalLength, aperture);
}
...
Material *SceneParser::parseMaterial() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    char texture_filename[MAX_PARSER_TOKEN_LENGTH];
